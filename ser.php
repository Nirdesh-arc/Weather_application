...
$createDatabase = "CREATE DATABASE IF NOT EXISTS prototype1";
...
// Select the database
mysqli_select_db($conn, 'prototype1');
...
$cityName = isset($_GET['q']) ? mysqli_real_escape_string($conn, $_GET['q']) : "Talladega";
...
if (!$result || mysqli_num_rows($result) == 0) {
    ...
    $apiKey = "d3e6fc7229c1690c12cd591a05a0f350";
    ...
    if (isset($data['main'])) {
        ...
        $iconCode = $data['weather'][0]['icon'];  // Get the icon code from the API response

        // Insert data into the database
        $insertData = "INSERT INTO weather (city, temperature, humidity, wind, pressure, icon_code)
                       VALUES ('$cityName', '$temperature', '$humidity', '$wind', '$pressure', '$iconCode')";
